...
import com.htstar.ovms.device.gateway.core.connector.api.ExchangeConnection;
import com.htstar.ovms.device.gateway.core.exception.LostConnectException;
...
public class TcpConnection<T> extends ExchangeConnection<T> {
    ...
    public void close() {
        ...
        cxt.close();
        logger.debug("当前连接以为关闭! ctx -> {}" , cxt.toString());
    }
    ...
    private void pushMessage0(T message) {
        try {
            if (message instanceof byte[]){
                ...
                ByteBuf encoded = cxt.alloc().buffer(bytemessage.length);
                encoded.writeBytes(bytemessage);
                ChannelFuture cf = cxt.writeAndFlush(encoded);
