    ...
    public void createTask(ApplyCarOrder order) {
        ...
        if (list.isEmpty()){
            ...
            for (Integer roleId :adlist){
                ...
                task.setProcessStatus(order.getProcessStatus());
                task.setVerifyRoleId(roleId);
                task.setVerifyType(NodeVerifyTypeConstant.ROLE_TASK);
                task.setTaskType(ProcessTaskTypeConstant.WAIT_ME);
            ...
            list.stream().distinct().forEach(
                    verifyUserId->{
                        ...
                        applyEventTask.setVerifyType(NodeVerifyTypeConstant.USER_TASK);
