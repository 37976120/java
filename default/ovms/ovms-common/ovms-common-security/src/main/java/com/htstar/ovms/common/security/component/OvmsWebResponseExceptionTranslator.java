...
import com.htstar.ovms.common.security.exception.*;
 ...
 */
@Slf4j
public class OvmsWebResponseExceptionTranslator implements WebResponseExceptionTranslator {
	...
	public ResponseEntity<OAuth2Exception> translate(Exception e) {
		...
		//自定义错误信息
		if (null != causeChain && causeChain[0].getCause() instanceof OvmsLoginException){
			return handleOAuth2Exception(new InvalidException(causeChain[0].getMessage(), null));
		}
	...
	private ResponseEntity<OAuth2Exception> handleOAuth2Exception(OAuth2Exception e) {
		...
		return new ResponseEntity<>(new OvmsAuth2Exception(e.getMessage(), e.getOAuth2ErrorCode()), headers,
