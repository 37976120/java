...
//    @PreAuthorize("@pms.hasPermission('admin_cardriverschedulesetting_add')" )
    ...
    @ApiOperation(value = "根据排班id查询该排班的车辆",notes = "查询可以申请的车辆")
    @PostMapping("/getSeetingLicCode/{settingId}")
    public R getSeetingLicCode(@PathVariable Integer settingId){
        return R.ok(carDriverScheduleSettingService.getSeetingLicCode( settingId));
    }
}
