...
//        Integer applyType = req.getApplyType();//默认公车申请
        Integer applyType = req.getApplyType() == null ? 0 : req.getApplyType();
        if (null == applyType) {
            return R.failed("请选择申请类型！！");
        }
        //1.得到企业当前运行的流程配置(如果没有配置，则新增一个默认配置)
        //默认配置：10，30，40，50，60五个节点
        ApplyCarProcess process;
        switch (applyType.intValue()) {
            //申请类型:0=公车申请(仅为)；1=私车公用；2=直接派车；
        ...
        ApplyVerifyNode nextNode = processService.getNextNode(
                ...
                order.getDriveType(),
                process.getId(),
                ProcessNodeTypeConstant.APPLY//当前所在节点，方法返回下一个节点
        );
        order.setNextNodeType(nextNode.getNodeType());
        //保存节点
        this.save(order);
        ApplyCarOrderDetail detail = new ApplyCarOrderDetail();
        BeanUtil.copyProperties(req, detail);//photoList照片列表[{photoUrl:'图片路径'}]  ccUserList[抄送人员ID列表(有序)]
        //转换一些前端传入的信息为JSON text
        /**
         * 外部乘车人列表[{name:'姓名(必填)',mobile:'手机号'}]
         */
        ...
        ApplyProcessRecord record = new ApplyProcessRecord();
        record.setOrderId(order.getOrderId());
        record.setOperationUserId(user.getId());//申请人ID
        record.setOperationType(ProcessOperationTypeConstant.AGREE);
        ...
        ApplyOrderTask task = new ApplyOrderTask();
        task.setOrderId(order.getOrderId());
        task.setTaskType(ProcessTaskTypeConstant.I_START);//审批类型：1=待我审批；2=已审批；3=抄送我的
        task.setVerifyUserId(user.getId());
        task.setReadFalg(1); //1:己读
        task.setProcessStatus(0);
        taskService.save(task);
        return R.ok();
    }
...
//        R r = carInfoFeign.updateApplyStatus(295);
    ...
    public R<Page<ApplyOrderPageVO>> getPage(PageApplyOrderReq req) {
        OvmsUser user = SecurityUtils.getUser();//当前操作用户
        req.setEtpId(user.getEtpId());//当前操作用户etpId
...
//            upEnt.setNextNodeType(ProcessNodeTypeConstant.FINISH);
    ...
    private R backOfficeCarApply(HandleOfficeCarApplyReq req) {
        ...
        //3.5修改车辆状态
        carInfoFeign.updateApplyStatus(order.getCarId());
        //4.生成操作记录
    ...
    R agreeOfficeCarApply(HandleOfficeCarApplyReq req) {
        /**
         * 分配车辆：{"operationType":1,"orderId":199}
         */
        OvmsUser user = SecurityUtils.getUser();
        ApplyCarOrder order = this.getById(req.getOrderId());
        ApplyCarOrderDetail detail = detailService.getById(req.getOrderId());
        ...
        //2.查看是否指定审批用户（不管怎样，指定还是什么的都在里面的）  如果是verify_type = 2我们需要的是verify_role_id
        //verify_type = 1：指定用户ID  AND task_type = 1:待我审批 from apply_order_task
        List<Integer> verifyUserList = nodeService.queryVerifyUserId(order);
        if (null == verifyUserList || verifyUserList.isEmpty()) {
            return R.failed("该申请已被审批");
        }
        ...
        if (order.getNowNodeType().equals(10)) {
            Integer carId = order.getCarId();
            try {
                carInfoFeign.updateApplyStatus(carId);
            } catch (Exception e) {
                log.info("提车修改车辆状态失败");
            }
        }
        ...
        //todo 节点在提车的时候设置当前OBD里程
        if (order.getNowNodeType().equals(30)) {
            updateOrder.setStaTime(LocalDateTime.now());
            Integer carId = order.getCarId();
            CarInfo carInfo = carInfoService.getById(carId);
            String initialMile = carInfo.getInitialMile();
            Double initialMileDouble = Double.valueOf(initialMile == null ? "0" : initialMile);
            try {
                R data = (R) deviceLastDataFeign.byCarId(carId.toString());
                Map deviceLastData = (Map) data.getData();
                Double currentTripMileage = Double.valueOf(deviceLastData.get("currentTripMileage").toString());
                Double total_trip_mileage = Double.valueOf(deviceLastData.get("totalTripMileage").toString());
                Double value = currentTripMileage + total_trip_mileage + initialMileDouble;
                updateOrder.setStartMile(value.toString());
            } catch (Exception e) {
                log.info("获取OBD里程失败");
            }
            //提车锁定车辆
        ...
        //TODO 还车节点操作
        if (order.getNowNodeType().equals(40)) {
            updateOrder.setEndTime(LocalDateTime.now());
            Integer carId = order.getCarId();
            CarInfo carInfo = carInfoService.getById(carId);
            String initialMile = carInfo.getInitialMile();
            Double initialMileDouble = Double.valueOf(initialMile == null ? "0" : initialMile);
            ...
            try {
                ...
                Double total_trip_mileage = Double.valueOf(deviceLastData.get("totalTripMileage").toString());
                Double value = currentTripMileage + total_trip_mileage + initialMileDouble;
                updateOrder.setEndMile(value.toString());
            } catch (Exception e) {
                log.info("获取OBD里程失败");
            }
        ...
        switch (order.getProcessStatus().intValue()) {
            ...
            case ProcessStatusConstant.WAIT_RECIVE:
                ...
                if (order.getCarId().intValue() > 0) {
                    ...
                    //TODO 增加用车人字段
                    expense.setCarUserId(order.getApplyUserId());
