...
//    }
     ...
     * @return
     */
    public static R pushMessageToApp(MsgPushStyleDTO msgPushStyle) {
        NotificationTemplate template = PushTemplate.getNotificationTemplate(msgPushStyle);
        AppMessage message = new AppMessage();
        message.setData(template);
        message.setOffline(true);
        message.setPushNetWorkType(0);
        message.setOfflineExpireTime(1 * 1000 * 3600);  //离线有效时间，单位为毫秒，可选
...
//        }
        ...
        appIdList.add(TargetTypeConstant.APP_ID);
...
//        IQueryResult personaTagResult = push.getPersonaTags(TargetTypeConstant.APP_ID);
...
//            ret = push.pushMessageToApp(message, "任务别名_toApp");
        ...
        if (ret != null) {
            ...
            getScheduleTask(ret.getResponse().get("contentId").toString());
            return R.ok(ret.getResponse().get("contentId").toString());

        } else {
            return R.failed("服务器响应异常");
        }
    }
    ...
    private static void getScheduleTask(String taskId) {
        IPushResult ret = push.getScheduleTask(taskId, TargetTypeConstant.APP_ID);
    ...
    private static void delScheduleTask(String taskId) {
        IPushResult ret = push.delScheduleTask(taskId, TargetTypeConstant.APP_ID);
