...
public class SysDictController {
	 ...
	 * @return 同类型字典
	 */
	@ApiOperation(value = "通过字典类型和字典警情查找警情类型", notes = "通过字典类型和字典警情查找警情类型")
	@GetMapping("/types/{value}")
	public SysDictItem getDictByTypeValue(@PathVariable("value") Integer value) {
		SysDictItem one = sysDictItemService.getOne(Wrappers
				.<SysDictItem>query().lambda()
				.eq(SysDictItem::getType, "alarm_type")
				.eq(SysDictItem::getValue, value.toString()));
		return one;
	}
	/**
	 * 添加字典
