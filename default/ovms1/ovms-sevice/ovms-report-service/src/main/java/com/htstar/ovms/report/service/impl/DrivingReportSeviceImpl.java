...
//            drvParams.put("endTime",endTime);
    ...
    /**
     * Description: 按人员统计用行驶
     * Date: 2020/11/18 11:43
     * Company: 航通星空
     * Modified By:
     */
    @Override
    public R<VceTotalPageVO> drivceMonthReport(ByDrivingReportPageReq req) {
        //获取当前登录用户
        OvmsUser user = SecurityUtils.getUser();
        req.setEtpId(user.getEtpId());
        //获取数据存入返回前端视图
        VceTotalPageVO vo = new VceTotalPageVO();
        //查询数据
        IPage<VceTotalVO> page = drivingReportMapper.queryDrivingMonthDrivingCount(req);
        vo.setPage(page);
        if(page !=null && page.getRecords().size() > 0){
            for (int i = 0; i < page.getRecords().size() ; i++) {
                VceTotalVO vo1 = page.getRecords().get(i);
        ...
        if (req.getExportStatus() == 1){
            return drivceReportExport(page,vo);
        }
        return R.ok(vo);
    }
    /**
     ...
     * @return
     */
    public R drivceReportExport(IPage<VceTotalVO> page,VceTotalPageVO vo) {
        HttpServletResponse response = WebUtils.getResponse();
        ExcelWriter writer =new ExcelWriter();
        writer.renameSheet("人员报表行驶记录");
        if(page ==null && page.getRecords().size() <= 0){
            return  null;
        }
